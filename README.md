...
งานนี้เป็นการเตรียมข้อมูลปริมาณน้ำฝนจากหลายไฟล์ให้อยู่ในรูปแบบที่พร้อมใช้งาน โดยเริ่มจากการรวมไฟล์ CSV หลายเดือน (ก.ค. – ธ.ค. 2025) จากหลายโฟลเดอร์ให้เป็น dataset เดียว จากนั้นคัดเลือกเฉพาะสถานีที่ต้องการใช้ตามไฟล์ stations

ตรวจสอบข้อมูลสูญหายโดยสร้างชุดข้อมูลที่ควรจะมี (ทุกสถานี × ทุกวัน) แล้วนำมาเทียบกับข้อมูลจริง เพื่อหา record ที่หายไป พร้อมคำนวณจำนวนและสัดส่วนของข้อมูลที่หายในแต่ละสถานี และตัดสถานีที่มีข้อมูลสูญหาย ≥ 20% ออก

ทำการเติมข้อมูลที่หายโดยใช้ค่า Median แยกตามสถานีและวันของปี (Day of Year) เพื่อรักษาลักษณะฤดูกาลของข้อมูล และลดผลกระทบจาก outliers หากยังมีค่าว่าง จะเติมด้วย Median ของทั้งสถานี

สุดท้าย นำข้อมูลที่สมบูรณ์มาคำนวณผลรวมปริมาณน้ำฝนรายเดือน แยกตามสถานี ปี และเดือน และบันทึกผลลัพธ์เป็นไฟล์สำหรับนำไปวิเคราะห์ต่อ
...
### **โจทย์**
เปรียบเทียบโมเดลพยากรณ์ปริมาณน้ำฝน เพื่อหาโมเดลที่ให้ผลลัพธ์ดีที่สุด
...
- ใช้ข้อมูล:
  - ปริมาณน้ำฝนหลายสถานี (cluster)
  - ตัวแปรสภาพอากาศ (climate features)  
- แบ่งข้อมูลแบบ Time Series → Train / Validation / Test  

### **ผลลัพธ์**
- ได้โมเดลที่แม่นยำที่สุดสำหรับพยากรณ์น้ำฝน  
- บันทึกผลเป็น CSV  
- แสดงกราฟค่าจริง vs ค่าทำนาย  

### **สรุป**
โมเดลแต่ละแบบให้ผลลัพธ์แตกต่างกัน โดยสามารถเลือกโมเดลที่มีประสิทธิภาพดีที่สุดจากค่า MSE, R² และ Accuracy เพื่อใช้ในการพยากรณ์ปริมาณน้ำฝนในอนาคต
---
